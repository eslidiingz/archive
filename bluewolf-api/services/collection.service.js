...
const methods = {
  ...
  findCollectionByTitle(title) {
    return new Promise(async (resolve, reject) => {
      try {
        const obj = await Collection.findOne({title: title} 
        );
        resolve(obj);
      } catch (error) {
        reject(ErrorNotFound("title: not found"));
      }
    });
  },
  ...
  findCollectionByAsset(id) {
    return new Promise(async (resolve, reject) => {
      try {
        const obj = await Collection.find({ assets: id  });
        resolve(obj);
      } catch (error) {
        reject(error);
      }
